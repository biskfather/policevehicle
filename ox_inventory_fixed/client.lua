...
local CraftingBenches = lib.load('data.crafting')
...
local function registerCommands()
	...
	for i = 1, 5 do
		lib.addKeybind({
			...
			description = locale('use_hotkey', i),
...
local function updateInventory(data, weight)
	...
	for i = 1, #data do
		...
		if not v.inventory or v.inventory == cache.serverId then
			...
			-- guard: skip malformed entries (no item data) instead of erroring
			if type(item) == 'table' and item.slot then
				if currentWeapon and currentWeapon.slot == item.slot then
				...
				print('^1[ox_inventory] updateSlots: skipped malformed entry (no item data)^0')
			end
		end
	end
